...
parser.add_argument('--data_dir', type=str, default='/media/weilegexiang/myfiles/CT-GAN-master/CT-GANs') #add your own path
...
mixed_output = lam * T.nnet.softmax(output_before_softmax_unl) + (1-lam) * T.nnet.softmax(training_targetsICT)
...
#loss_ICT = T.mean(lasagne.objectives.squared_error(mixed_output,output_before_softmax_unlm),axis = 1)
loss_ICT_ = 0.0*T.mean(loss_ICT,axis = 0)
...
for epoch in range(1200): #no learning rate decay. More epochs may give better result
    ...
    if epoch >= 400  and epoch%20 ==0:  # after 500 epochs, for every 100 epoch, change dataset.
        ...
        tempx = []
        tempy = []
        tempx.append(trainx_unl_org[np.max(training_targets3,axis = 1)>0.995]) 
        tempy.append(training_targets3[np.max(training_targets3,axis = 1)>0.995])
        tempx = np.squeeze(np.array(tempx))
        tempy = np.squeeze(np.array(tempy))
        tempy_onehot = tempy.argmax(axis = 1)
        #print(tempy.shape)
    ...
    training_target_varICT = training_targets[indices_all2]
